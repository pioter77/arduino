...
#include <DHT.h>
/*
 * middle volt divider pin to A7
 * cs pin from sd to D10
 * scL to a5
 * sdl to a4
 * joystick x to a2
 * joystick y to a3
 * battery sense to a6
 * dht pin to d9
 * analog rain sensor A0
 * base pin from emitter to d8
 * joystick switch d5
 * rain sensor vcc to d7
 * rain d0 pin to d6
 * main switch to d2
 * light sensor vcc to d3
 * 
 */

volatile bool by_time_started=0;
...
//bool first_run_condition=true;//variable to make the sensors readouts only at first run of loop in order not to slow down the manu response
bool sleep_state=false;//for putting ardu to sleep using joystick its switch push chanes it to true
bool sleep_state_timed=false;//similiar ike joystick but puttin  ardu after timed wakeup
...
const int light_sensor_vcc=3;//should be high when doing measurment then immediatelly low to save power!
...
if((counter>14)&&(by_time_started==1))//jak czsowo przerwanie i jak licznik jest wiekszy od ustalonej wartosci
{
  digitalWrite(joystick_vcc,HIGH);
    ...
    wdt_reset();
     counter=0;
     sleep_state_timed=true;
    by_time_started=0;
    Serial.println("wariant 1szy");
    //  ArduGoSleep();//going for sleep function
}else if((counter<=14)&&(by_time_started==1)){//jak czsowo przerwanie i jak licznik bedzie mniejszy od ustalonej wartosci
  Serial.println("wariant 2gi");
      wdt_reset();
  sleep_state_timed=true;
      by_time_started=0;
...
//amsensor_Readout(humi,temp);
 ...
 // digitalWrite(clock_vcc,LOW);
  ...
  ArduGoSleep();//going for sleep function
   ...
   if(sleep_state_timed==true){
   // Serial.println("petla czasowa sleep");
             wdt_reset();
    sleep_state_timed=false;
   // Serial.println("funkcja2");
       digitalWrite(joystick_vcc,LOW);
  ArduGoSleep();//going for sleep function
   }
...
void setWDT(byte sWDT)
{ //0b01000000
  WDTCSR = WDTCSR | B00011000;//no code between this! requires 4 data cycles max to be set succesfully
  WDTCSR = B00100001;//8 sec interval//masked?//no code between this
